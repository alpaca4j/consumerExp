...
class ConsumerExpKafkaResourceIT {
    ...
    void setup() {
        ...
        ConsumerExpKafkaResource kafkaResource = new ConsumerExpKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/consumer-exp-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/consumer-exp-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
